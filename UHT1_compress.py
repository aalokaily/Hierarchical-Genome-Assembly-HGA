...
def compute_freq_kmer(min_k, kmer_size, file, lease_base):
...
#convert the file of bits stings to ascii file and add the left over for each chromosome
    ...
    no_header_in_file = 0   # used to check of there is any header in the fasta file, if not create line that will store the leftover
    def_header = "" 
    left_over = ""
    p = 0
    for line in fin:
	if line[0] == ">":
		...
		while i < l :
			if l%8 == 0:
				if no_header_in_file == 0:
					left_over += "\n#UHT1_leftover:None:-1" + ":"
				else:
					left_over += def_header + ":"
				p += l
				break
			else:
				if l - i < 8:
					if no_header_in_file == 0:
						...
						left_over += def_header + ":" + line[i:].strip()
...
for i in range(1, partition+1):
	...
	#print "\nprocessing partition number " + str(i)
	compute_base_freq(genome_name + '_one_line_RLE_'+str(i)+'.fasta')

	bases_freq = []
    	for key in sorted(bases_freq_dict.keys()):
		bases_freq.append((bases_freq_dict[key], key))

   	bases_freq.sort(key=lambda x: x[0])
	if len(bases_freq) > 0:
		least_base = bases_freq[0][1]
	else:
		least_base = "!"
	#print bases_freq, least_base
	...
	compute_freq_kmer(min_k, k, genome_name + '_one_line_RLE_'+str(i)+'.fasta', least_base)
	...
	kmers_list = bases_freq + get_kmers_matching_hufman_design(kmer_sorted_freq_dict) + RLE_numbers_freq + Non_DNA_bases_freq
