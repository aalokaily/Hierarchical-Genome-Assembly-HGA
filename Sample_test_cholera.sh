...
wget http://ccb.jhu.edu/gage_b/datasets/V_cholerae_HiSeq.tar.gz | tee -a $curr/run.log

echo -----------unzip the dataset----------- | tee -a $curr/run.log

tar -xvf V_cholerae_HiSeq.tar.gz | tee -a $curr/run.log
...
echo -----------Interlace paired fastq files----------- | tee -a $curr/run.log

python interleave_fastq.py $curr/trimmed/reads_1.trimmed.fastq $curr/trimmed/reads_2.trimmed.fastq  data.fastq | tee -a $curr/run.log

rm -rf ./trimmed | tee -a $curr/run.log | tee -a $curr/run.log
rm -rf ./raw | tee -a $curr/run.log | tee -a $curr/run.log
rm V_cholerae_HiSeq.tar.gz | tee -a $curr/run.log | tee -a $curr/run.log
rm interleave_fastq.py | tee -a $curr/run.log | tee -a $curr/run.log
...
python $script_dir/HGA.py -velvet $curr/velvet_1.2.10 -spades $curr/SPAdes-3.0.0/bin -PA velvet  -P12  $curr/data.fastq  -R12 $curr/data.fastq -ins 335 -std 35 -P 4 -Pkmer 31 -Rkmer 51 -t 1 -out  ./HGA | tee -a $curr/run.log
